...
function formatAsTable(data) {
    ...
    const colWidths = headers.map(h => Math.max(h.length, ...data.map(row => String(row[h] || '').length)));
    const divider = colWidths.map(w => '-'.repeat(w + 2)).join('+');
    const headerRow = headers.map((h, i) => h.padEnd(colWidths[i])).join('  ');
    const rows = data.map(row => headers.map((h, i) => String(row[h] || '').padEnd(colWidths[i])).join('  '));
    return [headerRow, divider, ...rows].join('\n');
}
function formatAsJSON(data) {
    return JSON.stringify(data, null, 2);
}
function formatAsMarkdown(data, title) {
    if (data.length === 0)
        return `## ${title}\n\nNo data`;
    const headers = Object.keys(data[0]);
    let md = `## ${title}\n\n`;
...
// Commands
// ============================================
async function listSessions(client, config) {
...
async function listObservations(client, config, options) {
    const searchOptions = {
        agentId: config.agentId,
        limit: options.limit || 50,
    };
    if (options.type) {
        searchOptions.type = options.type.split(',');
    }
...
async function listMemories(client, config) {
    const results = await client.search({
        agentId: config.agentId,
        limit: 100,
    });
...
async function searchMemories(client, config, query) {
...
async function getContext(client, config, query) {
    const context = await client.getContext({
        query,
        maxTokens: 2000,
        includeRecent: true,
...
async function showStats(client, config) {
...
async function exportMemories(client, config, outputPath) {
    const results = await client.search({
        agentId: config.agentId,
        limit: 1000,
    });
...
async function showSessionDetails(client, config, sessionId) {
    const results = await client.search({
        agentId: config.agentId,
        limit: 100,
    });
...
async function main() {
    ...
    if (options.storagePath)
        config.storagePath = options.storagePath;
    ...
    try {
        switch (command) {
            ...
            case 'stats':
                ...
                break;
            case 'export':
                await exportMemories(memory, config, options.output);
                break;
            case 'session':
                if (positionalArgs.length === 0) {
