...
export class MemoryClient {
    ...
    async endSession(sessionId, input) {
        ...
        await this.config.hooks?.onSessionEnd?.(session, await this.store.getSummary(sessionId));
    ...
    async recordToolUse(toolName, input, result, sessionId) {
    ...
    extractObservationFromToolUse(toolName, input, result) {
        switch (toolName) {
            case 'BashTool':
            case 'bash':
    ...
    async getContext(input) {
        ...
        if (input.includeRecent) {
            const recent = await this.store.getRecentObservations(this.config.agentId, 10);
        ...
        if (input.includeImportant) {
            const important = await this.store.getImportantMemories(this.config.agentId, 0.7 // importance threshold
            );
    ...
    // Memory Extraction
    // ============================================
    async extractMemories(session, summary) {
