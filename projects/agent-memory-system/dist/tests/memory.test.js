...
// Test Configuration
// ============================================
function createTestConfig(agentId = 'test-agent') {
    return {
        agentId,
...
describe('MemoryClient', () => {
    ...
    describe('Observations', () => {
        ...
        test('should record observation with all types', async () => {
            const types = ['decision', 'bugfix', 'feature', 'discovery', 'change'];
        ...
        test('should record tool use observation for bash success', async () => {
            const obs = await memory.recordToolUse('BashTool', { command: 'ls -la' }, { exitCode: 0 }, session.id);
        ...
        test('should record tool use observation for bash failure', async () => {
            const obs = await memory.recordToolUse('BashTool', { command: 'exit 1' }, { exitCode: 1, stderr: 'Command failed' }, session.id);
            expect(obs).toBeDefined();
            expect(obs?.type).toBe('bugfix');
        });
        test('should record tool use observation for file write', async () => {
            const obs = await memory.recordToolUse('FileWriteTool', { path: 'test.ts', content: 'hello' }, { success: true }, session.id);
            expect(obs).toBeDefined();
            expect(obs?.type).toBe('change');
            expect(obs?.filesModified).toContain('test.ts');
        });
        test('should record tool use observation for file read', async () => {
            const obs = await memory.recordToolUse('FileReadTool', { path: 'test.ts' }, { content: 'file content' }, session.id);
            expect(obs).toBeDefined();
            expect(obs?.type).toBe('discovery');
            expect(obs?.filesRead).toContain('test.ts');
        });
        test('should return null for unknown tool', async () => {
            const obs = await memory.recordToolUse('UnknownTool', { data: 'test' }, { result: 'ok' }, session.id);
            expect(obs).toBeNull();
        });
    });
    // ============================================
