...
async function main() {
    ...
    // 4. Simulate tool use
    await memory.recordToolUse('bash', { command: 'python analyze.py --date-range 2026-03' }, { exitCode: 0, stdout: 'Analysis complete' }, session.id);
    await memory.recordToolUse('write_file', { path: 'reports/activity_analysis.md' }, { success: true }, session.id);
    // 5. End the session
