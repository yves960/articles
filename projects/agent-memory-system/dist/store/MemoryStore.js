...
export class MemoryStore {
    config;
    db; // Bun.SQLite or in-memory store
    // In-memory fallback
    sessions = new Map();
    ...
    async getRecentObservations(agentId, limit = 10) {
        if (this.db) {
    ...
    async getImportantMemories(agentId, threshold = 0.7) {
        if (this.db) {
            const rows = this.db.query(`
